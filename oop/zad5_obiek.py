...
class CashMachineIsFull(ValueError):
    pass
...
class CashMachine:
    ...
    def __init__(self):
        ...
        if len(self._money) > 10:
            raise ValueError("Za dużo banknotów")
    ...
    def put_money(self, money):
        if len(money) + len(self._money) > 10:
            raise ValueError("Chcesz wpłacić za dużo pieniędzy")
...
def test_cash_machine_full_money():
    bankomat = CashMachine
    with pytest.raises(CashMachineIsFull) as e:
        bankomat.put_money()
